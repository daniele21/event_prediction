...
def main():
    ...
    for index, row in X_test.iterrows():
        ...
        csv_validate_models_exact('gradient_boosting_exact.csv', home_team, away_team, home_win_prob,
                                  draw_prob, away_win_prob, test_data.loc[index, 'result_1X2'], quote_home,
                                  quote_draw, quote_way)
        csv_validate_models_double('gradient_boosting_double.csv', home_team, away_team, home_win_prob,
                                   draw_prob, away_win_prob, test_data.loc[index, 'result_1X2'], quote_home,
                                   quote_draw, quote_way)
...
if __name__ == "__main__":
    main()
    print(f"gradient_boosting_exact: {calculate_accuracy('gradient_boosting_exact.csv')}")
    print(f"gradient_boosting_double: {calculate_accuracy('gradient_boosting_double.csv')}")
    ...
    gain = calculate_gains_losses("gradient_boosting_exact.csv", value, min_prob, min_gain)
    print(f"accuracy_prediction: {calculate_accuracy('gradient_boosting_exact.csv')}")

    print("\nTotal gain with double results")
    gain = calculate_gains_losses("gradient_boosting_double.csv", value, min_prob, min_gain)
    print(f"accuracy_prediction: {calculate_accuracy('gradient_boosting_double.csv')}")

    print("\nTotal gain with exact results with kelly")
    gain = kelly_calculate_gains_losses("gradient_boosting_exact.csv", value, min_prob, min_gain)
    print(f"accuracy_prediction: {calculate_accuracy('gradient_boosting_exact.csv')}")

    print("\nTotal gain with double results with kelly")
    gain = kelly_calculate_gains_losses("gradient_boosting_double.csv", value, min_prob, min_gain)
    print(f"accuracy_prediction: {calculate_accuracy('gradient_boosting_double.csv')}")
