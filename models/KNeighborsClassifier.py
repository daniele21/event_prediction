...
def main():
    ...
    for index, row in X_test.iterrows():
        ...
        csv_validate_models_exact('KNeighbors_exact.csv', home_team, away_team, home_win_prob,
                                  draw_prob, away_win_prob, test_data.loc[index, 'result_1X2'], quote_home,
                                  quote_draw, quote_way)
        csv_validate_models_double('KNeighbors_double.csv', home_team, away_team, home_win_prob,
                                   draw_prob, away_win_prob, test_data.loc[index, 'result_1X2'], quote_home,
                                   quote_draw, quote_way)
...
if __name__ == "__main__":
    main()
    print(f"KNeighbors_exact: {calculate_accuracy('KNeighbors_exact.csv')}")
    print(f"KNeighbors_double: {calculate_accuracy('KNeighbors_double.csv')}")
    ...
    gain = calculate_gains_losses("KNeighbors_exact.csv", value, min_prob, min_gain)
    print(f"accuracy_prediction: {calculate_accuracy('KNeighbors_exact.csv')}")

    print("\nTotal gain with double results")
    gain = calculate_gains_losses("KNeighbors_double.csv", value, min_prob, min_gain)
    print(f"accuracy_prediction: {calculate_accuracy('KNeighbors_double.csv')}")

    print("\nTotal gain with exact results with kelly")
    gain = kelly_calculate_gains_losses("KNeighbors_exact.csv", value, min_prob, min_gain)
    print(f"accuracy_prediction: {calculate_accuracy('KNeighbors_exact.csv')}")

    print("\nTotal gain with double results with kelly")
    gain = kelly_calculate_gains_losses("KNeighbors_double.csv", value, min_prob, min_gain)
    print(f"accuracy_prediction: {calculate_accuracy('KNeighbors_double.csv')}")
